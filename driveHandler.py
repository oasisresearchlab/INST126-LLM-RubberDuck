...
#         # If a file with the same name exists, delete it to replace with a new one
#         if items:
#             file_id = items[0]['id']
#             print(f"Deleting existing file with name {filename} and ID {file_id}")
#             try:
#                 service.files().delete(fileId=file_id).execute()
#                 print(f"File {filename} deleted successfully.")
#             except Exception as e:
#                 print(f"Error deleting file {filename}: {e}")
#                 continue  # Skip this file if it can't be deleted
